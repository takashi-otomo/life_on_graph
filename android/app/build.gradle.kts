import java.io.FileInputStream
import java.util.Properties

plugins {
    id("com.android.application")
    // START: FlutterFire Configuration
    id("com.google.gms.google-services")
    // END: FlutterFire Configuration
    id("kotlin-android")
    // The Flutter Gradle Plugin must be applied after the Android and Kotlin Gradle plugins.
    id("dev.flutter.flutter-gradle-plugin")
}

// アップロード鍵 (keystore) 情報。`android/key.properties` があればそこから読み込む。
// CI ではこのファイルを Secrets から生成する (docs/release.md)。存在しなければ
// デバッグ署名にフォールバックし、ローカルの `flutter run --release` を妨げない (#121)。
val keystoreProperties = Properties()
val keystorePropertiesFile = rootProject.file("key.properties")
val hasReleaseSigning = keystorePropertiesFile.exists()
if (hasReleaseSigning) {
    keystoreProperties.load(FileInputStream(keystorePropertiesFile))
}

// versionCode は git のコミット数から自動採番する。コミットを重ねるごとに単調増加し、
// ローカル/CI のどちらでも同じ規則になるため、手動バンプ不要で Play の versionCode 重複も
// 避けられる。git が使えない場合は pubspec の値 (flutter.versionCode) にフォールバックする。
fun gitVersionCode(): Int? = try {
    val proc = ProcessBuilder("git", "rev-list", "--count", "HEAD")
        .directory(rootProject.projectDir)
        .start()
    val out = proc.inputStream.bufferedReader().readText().trim()
    if (proc.waitFor() == 0) out.toIntOrNull() else null
} catch (e: Exception) {
    null
}

android {
    namespace = "dev.otomo.life_on_graph"
    // Health Connect / 依存プラグイン (health, flutter_secure_storage 等) が要求する
    // AndroidX ライブラリのコンパイルに必要なため compileSdk を明示する。
    // 技術調査資料の推奨値は 34 だが、現行プラグイン群 (health 13 が依存する
    // androidx.health.connect:connect-client, flutter_secure_storage, device_info_plus)
    // が compileSdk 36 を要求するため 36 を採用する。
    compileSdk = 36
    ndkVersion = flutter.ndkVersion

    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }

    kotlinOptions {
        jvmTarget = JavaVersion.VERSION_17.toString()
    }

    defaultConfig {
        // TODO: Specify your own unique Application ID (https://developer.android.com/studio/build/application-id.html).
        applicationId = "dev.otomo.life_on_graph"
        // Health Connect SDK の動作要件 (Android 8.0 / API 26 以上) を満たすため
        // minSdk を 26 に固定する (技術調査資料 §2)。
        minSdk = 26
        targetSdk = 36
        // versionCode は git コミット数で自動採番 (フォールバック: pubspec)。
        versionCode = gitVersionCode() ?: flutter.versionCode
        versionName = flutter.versionName
    }

    signingConfigs {
        create("release") {
            if (hasReleaseSigning) {
                keyAlias = keystoreProperties["keyAlias"] as String
                keyPassword = keystoreProperties["keyPassword"] as String
                storeFile = file(keystoreProperties["storeFile"] as String)
                storePassword = keystoreProperties["storePassword"] as String
            }
        }
    }

    buildTypes {
        release {
            // key.properties があればアップロード鍵で署名する。無ければデバッグ署名に
            // フォールバックし、ローカルの `flutter run --release` を妨げない (#121)。
            signingConfig = if (hasReleaseSigning) {
                signingConfigs.getByName("release")
            } else {
                signingConfigs.getByName("debug")
            }
        }
    }
}

flutter {
    source = "../.."
}

dependencies {
    // local_auth (#79) の BiometricPrompt は API 26/27 の互換ダイアログで
    // AppCompat テーマを要求するため appcompat を明示的に追加する。
    implementation("androidx.appcompat:appcompat:1.7.0")
}
