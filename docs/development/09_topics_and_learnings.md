...
- バックグラウンド差分同期 (WorkManager) — 追加権限と Prominent Disclosure を伴う。
- iOS (HealthKit) 対応 — `health` 採用で経路は確保済み。
- 画面全体スワイプでの日/週/月移動、心拍グラフの時刻軸表示などの UX 改善。
- 表示設定 (テーマ・週開始)、対応言語の追加 (中国語簡体・韓国語・イタリア語)。
