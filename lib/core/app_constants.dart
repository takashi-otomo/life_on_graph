...
class AppConstants {
  ...
  /// 取得 → 保存 → 解放を繰り返してメモリ使用量を一定に抑える。心拍など高頻度データの
  /// 1 チャンクあたりの取得量・書き込み量を抑え、メインスレッドの長時間ブロック (ANR) を
  /// 避けるため小さめにする。
  static const int syncChunkDays = 7;

  /// 心拍専用の小さいチャンク日数 (#137)。
  ///
  /// 常時計測端末の心拍は 1 日でも数万〜十数万件になり、`health` プラグインが
  /// プラットフォームチャネルへシリアライズする際にメモリ上限 (OOM) を超えて落ちる。
  /// 1 回の取得・転送量を抑えるため心拍は他種別より細かく分割する。
  static const int heartRateChunkDays = 1;
